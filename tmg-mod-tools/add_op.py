...
class ADD_OT_SubDCube(bpy.types.Operator):
	"""Subdivided Cube Test"""
	bl_idname = 'mesh.add_ot_subd_cube'
	bl_label = 'Add Subdivided Cube'
	bl_description = 'Add a cube with subdivision settings.'
	bl_options = {'REGISTER', 'UNDO'}

	check_smooth_shading: bpy.props.BoolProperty(
	name="Shade Smooth",
	description="Set objects shading to smooth.",
	default=False
	)

	check_loc_to_cursor: bpy.props.BoolProperty(
	name="Location to Cursor",
	description="Set objects location to cursor else world center.",
	default=False
	)
	...
	x_translation: bpy.props.FloatProperty(
	name="X Location",
	description="Cube X location.",
	default=0,
	soft_min=-50.0,
	soft_max=50.0,
	unit='LENGTH',
	...
	y_translation: bpy.props.FloatProperty(
	name="Y Location",
	description="Cube Y location.",
	default=0,
	soft_min=-50.0,
	soft_max=50.0,
	unit='LENGTH',
	...
	z_translation: bpy.props.FloatProperty(
	name="Z Location",
	description="Cube Z location.",
	default=0,
	soft_min=-50.0,
	soft_max=50.0,
	unit='LENGTH',
	...
	def poll(cls, context):
		...
		# print(f"My location: {context.scene.cursor.location[0]}")
		# x_loc = context.scene.cursor.location[0]
	...
	def execute(self, context):
		if self.check_loc_to_cursor == True:
			bpy.ops.mesh.primitive_cube_add(size=self.size, calc_uvs=True, enter_editmode=True, align='WORLD', 
			location=(
					context.scene.cursor.location[0]+self.x_translation,
					context.scene.cursor.location[1]+self.y_translation, 
					context.scene.cursor.location[2]+self.z_translation
					))
		else:
			bpy.ops.mesh.primitive_cube_add(size=self.size, calc_uvs=True, enter_editmode=True, align='WORLD', 
			location=(
					0+self.x_translation,
					0+self.y_translation, 
					0+self.z_translation
					))
		
		if self.cuts > 0:
			bpy.ops.mesh.subdivide(number_cuts=self.cuts, smoothness=self.smoothness)
		
		if self.check_smooth_shading:
			bpy.ops.mesh.faces_shade_smooth()

		return {'FINISHED'}


class ADD_OT_SubDCylinder(bpy.types.Operator):
	"""Subdivided Cylinder Test"""
	bl_idname = 'mesh.add_ot_subd_cylinder'
	bl_label = 'Add Subdivided Cylinder'
	bl_description = 'Add a cylinder with subdivision settings.'
	bl_options = {'REGISTER', 'UNDO'}
	...
	def execute(self, context):
		bpy.ops.mesh.primitive_cylinder_add(vertices=self.vert_cuts, radius=self.size, depth=self.depth, enter_editmode=True, location=(0, 0, 0))
		if self.cuts > 0:
			bpy.ops.mesh.subdivide(number_cuts=self.cuts, smoothness=self.smoothness)
